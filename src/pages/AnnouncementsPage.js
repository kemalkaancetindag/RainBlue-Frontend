...
export default function Announcements() {
    ...
    useEffect(() => {
        axios.get(`${API_URL}/web/announcements`)
            .then(res => setAnnouncements(res.data.message))
                        ...
                        announcements ? (
                            <>
                                {announcements.map(ann => (
                                    <a href={`/announcement-detail?id=${ann._id}`} style={{ textDecoration: "none", color: "black" }} key={ann.SqlId}>
                                            ...
                                            <div style={{ width: "60%", height: "200px", borderRadius: "10px" }} className="border border-2">
                                                <div className="d-flex flex-row" style={{ height: "100%", width: "100%" }}>
                                                    <div className="d-flex flex-column justify-content-center align-items-center" style={{ width: "350px", height: "100%" }}>
                                                        <div style={{ width: "300px", height: "150px", backgroundColor: "grey", borderRadius: "5px", backgroundImage: `url(${API_URL}/images/${ann.image})`, backgroundSize: "cover" }} />
                                                    </div>
                                                    <div className="d-flex flex-column justify-content-around" style={{ padding: "20px" }}>
                                                        ...
                                                        <div>
                                                            <small>{Date(ann.createdAt).split(" ").slice(1, 5).join(" ")}</small>
                                                        </div>
